...
class CalendarToolHandler
{
    ...
    public function listAppointments(?string $start_date, ?string $end_date, ?int $limit = null, ?string $order = null): string
    {
        try {
            ...
            // If limit and order are specified, use next/last logic
            if ($limit !== null && $order !== null) {
                $now = \Carbon\Carbon::now();

                if ($order === 'upcoming') {
                    // Upcoming appointments
                    $schedules = $query->get()->filter(function ($schedule) use ($now) {
                        return $schedule->periods->contains(fn ($period) => $period->start_date_time->gte($now));
                    })
                    ...
                    // Recent appointments
                    $schedules = $query->get()->filter(function ($schedule) use ($now) {
                        return $schedule->periods->contains(fn ($period) => $period->end_date_time->lte($now));
                    })
                        ->sortByDesc(fn ($schedule) => $schedule->periods->max('end_date_time'))
                        ->take($limit);
                }
            } else {
                // Original logic with date filters
                $schedules = $query->get();
            ...
            // Custom message based on listing type
            if ($limit !== null && $order !== null) {
                $summary = $count === 0
    ...
    public function updateAppointment(?int $schedule_id, ?string $title, ?string $description, ?string $start_date, ?string $end_date, ?string $type): string
    {
        ...
        try {
            ...
            if (! empty($start_date) && ! empty($end_date)) {
                ...
                // Create new period with correct fields
